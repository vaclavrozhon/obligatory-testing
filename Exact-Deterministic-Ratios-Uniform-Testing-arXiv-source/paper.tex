...
$4/3$ for every $u\ge25/4$.  A separate uniform argument proves the same
sharp $4/3$ value for obligatory testing with unbounded, unannounced
processing times.
...
[add acknowledgements, thank bolzano (cite the paper) for proving a preliminary result (improving upon lower bound by the two taiwanese people in obligatory testing) and say the most followup work was mostly gpt5.6. ]

\section{Introduction and proof intuition}

This paper is broadly about the following problem. [explain testing] In another version of the problem, we replace testing by \emph{optimization} -- we can [explain u to pi optimization-of-running-time-angle]. 

In both cases, we are fighting the same problem -- we can get something good (knowledge / speed-up) but it also costs us precious time.  How to deal with this conflict is the main point of this paper. 

While both being able to guess running time of procedures and being able to optimize them is clearly important, our goal is to come up with a suitable "metaalgorithm". Ideally, we want a simple algorithm that is a "wrapper" that around guessing/optimization heuristics. 
...
\subsection{Warmpu -- obligatory testing}

We start with the following, simplest to understand, setting. [explain obligatory, mention nonpreemptive is for the whole paper]

[explain competitive analysis]

This model has been introduced by XX who proved XX. [explain what is known]

Our first contribution is settling the right competitive ratio in both deterministic and randomized model [some footnote about how adaptive adversary is basically the same as deterministic model ]. 

[formal statement of deterministic result, informal, tag it as "informal version of THm XX"]

[the same for randomized]

[next to each other -- simplified/intuitively explained algorithms used in those two results]

Let us explain the two algorithms. [brief explanation here]. 



\subsection{Instance optimality}

We still consider the same problem. 

The result from previous section is still not satisfying. In practice, it is likely that many times, we will be in a very degenerate setting. E.g., the testing is so cheap that first testing everything and then sorting is essentially 1-competitive. Our algorithm is howerver still only proven to be 1.58 / 1.33 competitive. 

I.o. comes to the rescue. [explain the concept] This is actually the same idea behind online algos themeselvees -- instead of measuring cost, we compare ourselves to offline optimum. But here, in the "outer loop" we compare ourselves also to all other online algorithms. 

[now explain why we cannot have i.o. in the deterministic setup]

But we can have i.o. in the randomized setup. We still need to be careful because very long job can screw us, so results are up to $o_L(n^2)$. 
...
[simplified algo (inside algo environment)]

Tthe following graph where we plot the best competitive ratio achievable based on the upper bound L (or did we call it $u$)? 


\subsection{Dropping obligatority}

When we think about testing, the assumption we have to test is not very satisfying -- in many scnarios, the tests will not reveal enough to speed stuff up anyway and they can substantially slow down execution. 

The problem is that without obligation to test, it is harder to prove results. [explain briefly why deterministic algos cannot work, write it down more carefully in some appendix and point to it]. For randomized algorithms, we have simiilar problem. 

However, instance optimality comes to the rescue. We can still prove that there is an instance optimal algo. 

[statement]

[algorithm]

[explain something about it, e.g what happens when tests take very long time to be rendered useless]



\subsection{Optimalization}

We can also think about "tests" as being about speeding up execution -- without them, jobs take some uniform long time $u$ to be processed. 

We want to get algorithm that works for whatever value of $u$ (we will assume it's known since in this simplification we anyway learn it by running a single job). We notice that this generalizes obligatoriy testing since for $u \to \infty$ we recover the setup from our warmup. 

We get the following two curves for both det and rand algorithms. 

[two pictures with curves, draw them nicely in tikz]

you can notice the curves converge to 1.58 from thm XX and 1.33 from thm XX respectively. 

[write the informal result, it says something like "the ratio is as in figure XX" instead of defining it]

Some discussion about the algorithms here, put them again to algo environment and simplified. 
...
of the resulting deterministic curve.  Against an oblivious adversary, our
random-labelling analysis gives the separate exact four-piece curve
\eqref{eq:intro-random-common-upper-curve}; its survival-function reduction
shows that binary distributions already witness every worst branch.
