...
\title{Scheduling with Uniform Tests:\\
Exact Ratios and Instance-Optimal Randomized Policies}
...
\begin{abstract}
We give a unified account of three single-machine scheduling models with unit
tests and total completion time.  First, in the \emph{common-upper} model a
job may be executed untested for a common cap $u$, or tested and then
processed for its revealed time.  We determine the exact deterministic
size-asymptotic competitive ratio for every $u\in(0,\infty]$, where
$u=\infty$ denotes obligatory testing.  The obligatory value is
\[
 R_{\rm obl}=1+r=1.570574096649395\ldots,
 \qquad r=\frac{2}{z_\star-1},\qquad z_\star-3=\log z_\star.
\]
For finite $u$, the exact curve has six explicit regimes and stabilizes at
$R_{\rm obl}$ for every $u\ge z_\star$; its global maximum is
\[
 \sup_{u>0}V(u)=\frac{1+\sqrt{3+2\sqrt5}}2
 =1.866760399173862\ldots .
\]

Second, we settle randomization for obligatory testing against an oblivious
adversary.  The algorithm is not told the multiset and processing times need
not be bounded.  A sublinear sample learns a maximum-density threshold, and
a tail-safe fallback handles missed large values.  We prove
\[
 \EE\ALG\le \frac43\OPT+20378n^{7/4},
\]
and a uniformly labelled multiset with half zeros and half twos gives the
matching ratio $4/3-O(n^{-1/2})$.  Thus the sharp randomized asymptotic ratio
is $4/3$.

Third, we study a distinct \emph{blind-execution} model: an untested job may
be run for its actual processing time, while testing costs one unit and
allows the revealed processing to be delayed.  For every fixed bound $L$, we
identify an explicit distributional benchmark $\Phi(D)$ and prove a uniform
instance-optimal theorem at the $n^2$ scale.  One universal policy, not told
the multiset, attains $n^2\Phi(D_M)+o_L(n^2)$ on every fixed oblivious
labelling, whereas even a policy told the multiset incurs
$n^2\Phi(D_M)-o_L(n^2)$ on some fixed labelling.  The canonical policy has a
threshold-testing prefix, a known-medium SPT block, a blind block, and a
known-high SPT tail; its tested fraction minimizes an explicit quadratic.
The deterministic exact curve, randomized $4/3$ theorem, and bounded
instance-optimal theorem are formalized in Lean~4.
\end{abstract}
