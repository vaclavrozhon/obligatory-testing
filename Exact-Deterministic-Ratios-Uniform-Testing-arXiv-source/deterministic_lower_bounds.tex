...
\subsubsection{The harmonic core below the raw cap}
\label{sec:lower}
...
two.  Consequently, whenever the construction is later used with a raw cap
$u>3$, every job satisfies $1+p_j<u$ and the cap does not alter its offline
effective length.  Its notation is local to this subsection.
...
Prepend a fraction $m$ of capped jobs with value $p=u$ to a later cap-free
block occupying the remaining fraction $s=1-m$ and having total normalized
...
\begin{proof}
A capped job completed after all caps have been tested but before a later job
of value $p$ contributes $1+u$ to their pairwise online cost.  If we postpone
...
It remains to cover capped jobs completed while the initial cap block is
still being tested.  Let $q$ be their fraction of the full instance.  Their
pairs with all later jobs add
$q[s(u-1)-L]$.  Their largest possible saving against not-yet-tested caps is
...
\begin{proof}
A pair consisting of one initial cap and one later job of value $p$
...
To compare with the cap-free lower-bound construction, run a virtual copy of
the revealing algorithm.  If this virtual algorithm later runs a fresh job
of assigned value $p$ raw, the cap-free simulation instead tests and
...
Inductively every simulated cap-free completion is no later than the
corresponding virtual revealing completion.  Since the
finite-cap and cap-free offline lengths are both $1+p$ on the later block,
write $\ALG_{\rm hyb}$ and $\OPT_{\rm hyb}$ for the online and offline costs
of this physical cap-free simulation.  Then
\begin{equation}
...
optima coincide.  Thus we obtain the following direct comparison.  For
