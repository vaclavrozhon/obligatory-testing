...
\medskip
\noindent\textbf{Claim 1 (Two cases for the minimizing $b$).}
...
There are therefore two quadratic formulas.  When $uy-(u-1)>0$,
substitution gives
\begin{equation}
 \min_b f_{u,R}(y,b)
...
\cref{eq:opt:Rquad-equation}.  When $uy-(u-1)\le0$, the minimizing choice is
$b=0$, and
\begin{equation}
 \min_b f_{u,R}(y,b)
 =1-R+2y+(u-1)(1-R)y^2.
...
\cref{eq:opt:zero-b-tangency}.  Finally, $\RdetRO(u)\ge1$ is trivial for
$u\le1$.
...
$\alpha<e-1$ in \cref{eq:lower-span} holds at the maximizer.
...
argument from Claim~1 in the proof of \cref{thm:lower}: assign each revealed answer to
...
\subsubsection{Combining capped jobs with a harmonic block}
...
\cref{eq:opt:combined-A,eq:opt:combined-O} gives the guaranteed two-parameter
lower ratio
\begin{equation}
 \mathcal R_u(m,t)=1+
...
\end{equation}
\medskip
\noindent\textbf{Claim 2 ($(c,m)$-parametrization).}
...
As $c$ decreases from $1/\varphi$ to $r$, the parameters run from
\begin{equation}
...
\cref{lem:opt:cap-quota}, this proves the proposition.
The construction applies directly when
$\uDet{4}<u<\uDet{5}$.  The lower bounds at $u=\uDet{4}$ and
$u=\uDet{5}$ follow by taking limits from this open interval.
\end{proof}
