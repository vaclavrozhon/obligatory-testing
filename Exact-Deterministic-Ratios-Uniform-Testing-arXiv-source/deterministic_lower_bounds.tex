...
instances by the answer-freezing argument at the end of its proof.
\end{proof}
...
Multiplying every normalized group size by a common factor multiplies the
phase accounting, harmonic cancellation, and completion of squares in
\cref{eq:lower-accounting,eq:lower-telescope,eq:lower-QK} are multiplied by
...
answer-freezing argument from the theorem.  This produces arbitrarily large
fixed rational instances with the stated limit and liminf bounds.
\end{proof}
...
family is tight.  Here a \emph{capped} job has $p=u$, while a block is
\emph{cap-free} if every job in it satisfies $1+p\le u$.  The correct
adversary puts a capped block before a scaled harmonic core: it first reveals
a prescribed fraction of jobs with $p=u$ and then reveals the cap-free
multilevel instance from the preceding subsection.
Two lemmas show that neither raw execution nor early processing of the capped
jobs can avoid the resulting lower bound.
...
Prepend a fraction $m$ of capped jobs (that is, jobs with $p=u$) to a later
cap-free block occupying the remaining fraction $s=1-m$ and having total normalized
processing time $L$.  Suppose every later value satisfies
$1+p\le u$ and
\begin{equation}
 s(u-1)-L-m\ge0.
...
\begin{proof}
A capped job completed after all jobs with $p=u$ have been tested but before
a later job of value $p$ contributes $1+u$ to their pairwise online cost.  If we postpone
the capped job until after the later job, the same pair contributes $2+p$.
The postponement lowers the cost by $u-1-p\ge0$, so the postponed schedule is
a valid lower bound on the algorithm's actual cost.

It remains to cover capped jobs completed while the initial block of
$p=u$ jobs is still being tested.  Let $q$ be their fraction of the full
instance.  Their
...
online coefficient, and total normalized positive processing time of the later
cap-free block.  After postponing all initial capped jobs, the corresponding
...
contributes $2+p$ online and $1+p$ offline.  Summed over capped fraction $m$
and later fraction $1-m$, these give the cross terms involving $m(1-m)$ and
$mL$.
A pair of capped jobs contributes $2+u$ online and $u$ offline, which gives
the terms proportional to $m^2$.  Adding the cap-free coefficients yields
...
there are fixed instances whose asymptotic excess is at least the combined
coefficient $A^{\rm lb}-R\cdot O$ obtained from the capped prefix and
the later block in
...
$S-\ell$ first touches, absorbing the one-job quota overshoot and integer
rounding into its zero group.  Fix an order of the job types in this later
...
initial capped jobs, and $t$ selects the scaled harmonic block that follows.
...
common integer $H$.  For a fixed number of positive levels, quota overshoot,
one-job
terms, and rounding are $O_u(H)$ whereas the displayed pair costs are
$\Theta(H^2)$.  Let $H\to\infty$ first, then increase the harmonic depth, and finally
...
\subsubsection{The plateau lower bound by a below-cap transfer}
...
\begin{lemma}[Bounded below-cap transfer]
