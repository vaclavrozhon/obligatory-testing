...
[tady chci minipage a v ní dva algoritmy vedle sebe. zkus oba algoritmy napsat vic dopodrobna]
[spt je nedefinovane]
[]
\begin{algorithm}[H]
\caption{The two optimal obligatory-testing policies, informally}
\label{alg:intro-obligatory}
\begin{minipage}[t]{0.48\textwidth}
\textbf{Deterministic: adapt the threshold.}
\begin{algorithmic}
\State Test the jobs in any fixed order.
\State Before each test, update a threshold from the numbers of positive and
deferred jobs seen so far.
\State Process the new job now if it is below the threshold; otherwise defer
it.
...
\begin{algorithmic}
\State Test a sublinear uniform random sample and keep its positive jobs
pending.
\State Learn one threshold; if the sample is inconclusive, use a safe
test-all-then-SPT fallback.
\State Otherwise process the sampled jobs at or below the threshold.
\State Test the remaining jobs in random order; process a job now exactly
when it is at or below the threshold.
\State Finish all deferred jobs shortest first.
\end{algorithmic}
\end{minipage}
\end{algorithm}
...
$1.57\ldots$ (we prove this formally in XX). 
...
length $u$, while a test replaces $u$ by a smaller value $p_i\leq u$. This second interpretation corresponds to the tests actually being optimizations -- for example, one can imagine a basic code that is being run that typically solves a task in $u$ units of time; but if we know details of the job, we can ask an AI agent to adapt the code before running it, perhaps adding some input-tailored heuristics that will make it faster. 

Second, does the test (optimization) reveal $p_i$?  If it does, the scheduler gains both a faster job and information about where the job belongs.  If it does not, the
job becomes faster but that speedup remains unknown until it finishes.
...
\end{tabular}
\end{center}
[neni logictejsi mit blind optimization pred common upper bound?]
...
both. [a few words about whether somebody defined those models]

The next three subsections describe these three models. 

\subsection{One algorithm for every bag}

Once testing becomes optional, a single worst-case ratio as in thm1.1 can no longer tell the
whole story: 

[tady mi to prijde divny, neni lepsi napsat ze problem jsou dlouhe joby a dat kratky priklad? ]
Consider the input as a bag $M$ of $n$ job lengths.  On one bag
