...
[tohle chci aby byly dva algorithm ne jeden!]
\begin{algorithm}[H]
\caption{The two optimal obligatory-testing policies}
\label{alg:intro-obligatory}
\begin{minipage}[t]{0.48\textwidth}
\raggedright
\textbf{Deterministic: adapt the threshold.}
\begin{algorithmic}
\State Let $\rho=0.57...$ and $N=D=0$.
\State Before a test, let $x$ be the number of untested jobs and put
$y=(D-RN)/x$. [proc pouzivame x N D na tri veci co se sectou na n? to zni jako spatna notace. nebylo by lepsi na zacatek napsat neco jako "v i-tem kroku vzdy mame n = d_i + p_i + (n-i) nebo tak neco? ]
\State Use threshold $T_i = 1$ if $y\le-1$; otherwise use
\[
  T_i 1+\frac12\log\frac{\rho+2}{\rho-2y}.
\]
\State Test the next job.  Process it now if it is at or below the threshold $T_i$;
otherwise defer it.
\State Update $N$ (positive outcomes) and $D$ (deferred outcomes).
\State Finish all deferred jobs shortest first.
\end{algorithmic}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\raggedright
\textbf{Randomized: learn the threshold.}
\begin{algorithmic}
\State Shuffle the jobs and test the first $o(n)$ of them to estimate the job-length distribution. 
\State Let $\rho = ... max \widehat a/(1+\widehat m)$ where we maximize over ... 
\State If $\rho < 1$ [radsi pouzijme jednoduche konstanty kdyz to nic nemeni], test every job and finish all positive [tohle dropni, nechceme travit cas s length=0 jobs v intru vubec!] jobs shortest first.
\State Otherwise use $T_i = 1/ \rho_i$. [proc vubec definujeme $\rho_i$?] Process the
sampled jobs at or below it and defer the other sampled jobs.
\State Test the rest in random order, processing low outcomes now and
deferring the others.
\State Finish all deferred jobs shortest first.
\end{algorithmic}
\end{minipage}
\end{algorithm}
...
After one common principle, we return to these three models in this order.

\subsection{One algorithm for every bag of jobs}

Once testing becomes optional, the ordinary competitive ratio gets problematic since in blind execution it can even be unbounded:  Take a
bag of jobs with containing many zero jobs and one long job.  A clairvoyant scheduler completes
all zero jobs for free and runs the long job last.  An online scheduler that
encounters the long job early delays every zero that remains.  Avoiding this
mistake by testing many jobs can itself cost on the order of $n^2$.  Thus the
ratio to the clairvoyant optimum may be huge even though there is still a
well-defined best online way to handle this particular bag.

[tady to prepis jak jsme se o tom bavili]

This leads to a stronger goal than measuring competitive ratio.  For each bag $M$,
...
\begin{theorem}[One algorithm for every bag, informal version of
\protect\cref{thm:common-upper-instance,thm:blind-instance,thm:bo-instance}]
\label{thm:intro-instance-optimality}
Fix a bound on the job lengths.  In each of the three optional models [wtf?? kazdy teorem ma byt jen pro jeden model] there
...
\begin{algorithm}[H]
\caption{One algorithm for every bag}
\label{alg:intro-every-bag}
\begin{algorithmic}
\State Privately shuffle the jobs and observe a sublinear pilot sample.
\State Use the sample to estimate the bag of lengths.
\State Pretend that this estimate is exact and compute the threshold, tested
fraction, and block order minimizing the model's explicit cost.
\State Run the resulting policy on the remaining randomly ordered jobs.
\end{algorithmic}
\end{algorithm}
[rozepis ten algoritmus a napis jak se lisi od toho z predchozi subsekce]
...
The example from the preceding subsection explains why we compare with the
bag-dependent value $\Phi(M)$ rather than insisting on a bounded ratio to the
clairvoyant optimum.  The quantity $\Phi(M)$ captures the best cost that
online information permits on this particular bag.
...
Next suppose that a unit-time optimization replaces $u$ by $p_i\leq u$ but
does not reveal $p_i$.  The value becomes known only when the optimized job
finishes.  A threshold can no longer separate short and long outcomes:
...
This policy is instance optimal up to $o(n^2)$.  Its worst-case ratio,
however, grows as $\sqrt u/2$.  The difference between this model and the
revealing common-upper model below shows exactly what information is worth:
speedup alone does not give a bounded ratio when $u$ grows.
...
Finally, assume that every untested job can be run for a known raw time $u$.
