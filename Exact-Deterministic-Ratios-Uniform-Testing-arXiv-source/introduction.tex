...
Suppose that we have many jobs waiting for one machine.  The length of each
job is written on a folded piece of paper.  If we knew all the lengths, the
right schedule would be clear: we would simply run the jobs from shortest to
longest.  But the papers are folded, and opening one of them takes one unit of
machine time.
...
than four new sets of notation; in every row, testing costs one unit of
machine time.
...
\toprule
model & without a test & what a test does \\
\midrule
...
The two middle rows separate the two benefits of a test.  Blind execution
studies information without speedup.  Blind optimization studies speedup
without advance information.  The common-upper model contains both.
...
jobs.  A scheduler that knows the bag's labelling completes all zeros at time
