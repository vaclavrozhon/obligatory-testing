...
opening a paper adds one hundred units to the sum of completion times.  Hence, the machine has to balance two uses of its time: it can learn which jobs are
short, or it can complete jobs that are already understood.

This is the scheduling problem studied in this paper, variants of which were already studied [few citations here].  We start with its
...
three natural variants.  The same simple ideas---random sampling,
and shortest-first execution, thresholding---give exact answers in all of them.

\subsection{First example: every job has to be tested}

There are $n$ jobs.  Testing job $i$ takes one unit of time and reveals a number $p_i\geq0$.  The job then needs another $p_i$ units of processing.  For now, every job has to be tested, i.e., testing is \emph{obligatory}.


The obligatory-testing model (see XX for formal definition) for total completion time was introduced by
...
\begin{theorem}[Obligatory testing, informal version of  \cref{thm:det-obligatory,thm:rand-obligatory}] % takhle to udelej i u ostatnich informal theoremu
\label{thm:intro-obligatory}
In the obligatory-testing model, the best achievable deterministic asymptotic competitive ratio is
$1.57\ldots$ by algorithm XX.  Against an adversary that fixes the input before the algorithm draws its random bits, the best randomized ratio is $4/3$ by algorithm XX.
\end{theorem}

Let us briefly describe the two algorithms. 

[sem dej vedle sebe informal verze obou algoritmu]


The general idea is to have a threshold algorithm that runs short jobs as soon as possible and leaves long jobs til the end. 
...
$1.5705740966\ldots$. % tyhle cisla staci vzdy na dve desetinna mista
