...
\begin{algorithmic}
\State Let $R$ be the optimal ratio ($R=1.57\ldots$), and put $\rho=R-1$.
Start with $N=D=0$.
\State Before a test, let $x$ be the number of untested jobs and put
$y=(D-RN)/x$.
\State Use threshold $1$ if $y\le-1$; otherwise use
\[
  1+\frac12\log\frac{\rho+2}{\rho-2y}.
\]
\State Test the next job.  Process it now if it is at or below the threshold;
otherwise defer it.
...
\begin{algorithmic}
\State Shuffle the jobs and test the first $n^{3/4}$ of them.
\State Round sampled values in $[0,32]$ into $n^{1/4}$ bins; keep larger
values in an overflow bin.
\State For each finite-bin prefix, let $\widehat a$ be its sample mass and
$\widehat m$ its normalized rounded work.  Maximize
$\widehat a/(1+\widehat m)$.
\State If this density is below $1/16$, test every job and finish all positive
jobs shortest first.
\State Otherwise use the reciprocal density as a threshold.  Process the
...
Once testing becomes optional, the ordinary competitive ratio is not the
only natural question.  In blind execution it can even be unbounded.  Take a
bag with many zero jobs and one long job.  A clairvoyant scheduler completes
...
Write $M$ for the bag of $n$ job lengths.  On one bag it may be best to test
almost every job; on another it may be best to test none.  We would like one
algorithm to make the right choice on each bag, not merely to survive the
worst bag.
