\section{Finite caps: the exact curve}
\label{sec:optional}

The goal of this section is to prove \cref{thm:optional-curve}, the rigorous
deterministic part of the informal common-upper curve theorem
\cref{thm:intro-common-upper-curves}.  We now move from the cap-free endpoint
to finite $u$.  Raw execution becomes
available, all jobs have the same known upper limit, and a test reveals
$p_j\in[0,u]$.  Thus the offline effective length of job $j$ is
\begin{equation}
 q_j=\min\{u,1+p_j\},
...
\[
 \ALG_{\mathcal A}(I)\ge\bigl(V(u)-\epsilon\bigr)\OPT(I).
\]
\end{theorem}
...
and identical long jobs may be relabelled so that the $e$ completed ones are
...
size-asymptotic value at the cap-free endpoint, restricted to
$0\le p_j\le P$.  Then
\begin{equation}
 V(u)\ge V_\infty(u-1).
