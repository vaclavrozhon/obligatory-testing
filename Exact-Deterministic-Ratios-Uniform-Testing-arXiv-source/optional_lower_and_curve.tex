...
is $q_u(p_j)=\min\{u,1+p_j\}$, and the offline optimum schedules these
effective lengths in nondecreasing order.  We write $V(u)$ for the optimal
...
when $u>1$, every effective length $q_u(p_j)$ from
...
$1+1/\sqrt{u-1}$ on $[u_0,\varphi+2]$ and $\varphi$ on
$[\varphi+2,\infty)$.
\end{lemma}
