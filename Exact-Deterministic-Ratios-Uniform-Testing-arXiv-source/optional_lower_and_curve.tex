...
\end{equation}
For $\varphi+2\le u\le \zstar$, define $\Cmix(u)=1+c$ by the unique
solution $(c,m)$ of
\begin{equation}
...
\rho_I(u), &\udiam\le u\le \uzero,\\[1mm]
\displaystyle 1+\frac1{\sqrt{u-1}},
   &\uzero\le u\le\varphi+2,\\[3mm]
\Cmix(u), &\varphi+2\le u\le \zstar,\\[1mm]
\RdetOT, &u\ge \zstar.
\end{cases}}
...
\[
 \cost(\mathcal A,I)
 \ge\bigl(\RdetRO(u)-\epsilon\bigr)\cost(\OPT,I).
\]
...
$(u-1)^3\ge u^2$, with equality at $u=\uzero$.
...
\begin{equation}
 [u(u-1)]^2-u(u-1)-1\le0,
\end{equation}
whose positive equality is $u=\udiam$.  We have proved the following
...
\begin{equation}
 ...
 u,&1\le u\le \udiam,\\
 \rho_I(u),&\udiam\le u\le \uzero,\\[1mm]
 \displaystyle1+\frac1{\sqrt{u-1}},&u\ge \uzero.
 \end{cases}
...
\begin{proof}
For $1<u\le \udiam$, use $\rho=u$ and the maximizing point of
\cref{eq:opt:stopping-positive-branch}.  For
$\udiam\le u\le \uzero$, use the positive root $\rho_I(u)$ and
...
at $u=\uzero$.  For $u\ge \uzero$, use the zero-$b$ tangency point from
...
For $u\le \udiam$, use \textsc{Raw}.  It is optimal when $u\le1$;
when $u>1$, every effective length $\lambda_u(p_j)$ from
\cref{eq:optional-effective-length} is at least one, so the ratio is at most
$u$.
...
For $\udiam\le u\le2$, arbitrary values reduce to binary values by
...
$\mathbb E\min(\lambda_i',\lambda_j')\le\min(\lambda_i,\lambda_j)$.  Thus some binary realization
has competitive excess at least that of the original instance.

It remains to justify arbitrary values for $2\le u\le \uzero$.  The following
quadratic lemma records the entire endpoint calculation.

\begin{lemma}[UTE endpoint game]
\label{lem:opt:ute-endpoint-game}
For $2\le u\le \uzero$, UTE with the parameters
...
At $u=\uzero$, $b=0$.  The same zero-prefix algorithm remains exact through
$u=\varphi+2$ and continues with a $\varphi$ guarantee afterwards.  We state
the continuation because it makes the three-policy lower envelope in
\cref{fig:intro-curves-det} literal rather than schematic.

\begin{lemma}[Zero-prefix extension]
\label{lem:opt:zero-prefix}
For every $u\ge \uzero$,
...
$1+1/\sqrt{u-1}$ on $[\uzero,\varphi+2]$ and $\varphi$ on
$[\varphi+2,\infty)$.
\end{lemma}
...
$u\le\varphi+2$.  Binary instances attain the second value.
\end{proof}
...
For $\varphi+2<u<\zstar$, neither the binary family nor the pure cap-free
...
hybrid benchmark $A^{\rm lb}-RO$ from
...
\begin{equation}
 ...
 \quad\text{to}\quad
 (u,m,t)=(\zstar,0,\sqrt{\zstar}).
 \label{eq:opt:mixed-endpoints}
\end{equation}
It satisfies $u\ge\varphi+2$, $\ln t<1$, and $m\le1/\varphi$.  Therefore
\begin{equation}
 (1-m)(u-1-\ln t)
...
$u\in[\varphi+2,\zstar]$ has exactly one parameter pair.
...
\begin{equation}
 \RdetRO(u)\ge \Cmix(u)
 \qquad(\varphi+2\le u\le \zstar).
 \label{eq:opt:mixed-lower-final}
\end{equation}
The construction above applies directly in the open interval.  At
$u=\varphi+2$ and $u=\zstar$ the same statement follows by approaching from
...
\begin{equation}
 1+\ln(1+y_*)
 =1+\frac12\ln \zstar
 =\frac{\zstar-1}{2}=\frac1r,
 \qquad y_*=\sqrt{\zstar}-1.
\end{equation}
