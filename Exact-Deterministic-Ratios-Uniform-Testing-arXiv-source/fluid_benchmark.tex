\section{The finite fluid benchmark}
...
instance-optimality sections justify the corresponding approximation for
actual jobs.  The present section contains no sampling or limiting argument:
it solves the finite-dimensional fluid problem exactly when the distribution
is known.
...
\subsection{How the three models use the benchmark}
\label{sec:fluid-specializations}

The theorem deliberately fixes $D$, $q$, and $v$.  Later sections perform
only model-specific algebra and the transfer to actual shuffled jobs.
...
that one concentration event controls them simultaneously.  The later
instance-optimality proofs show separately that every discrete competitor
induces a fluid state obeying \cref{eq:common-fluid-envelope}, up to
$o(1)$ normalized error, and that a sampled finite algorithm implements
$\textnormal{\textsc{FluidSPT}}$ with the same accuracy.  Those transfer
arguments are not assumptions of the fluid theorem above.
