...
$R=1+c$, and we retain $\varphi$, $\zstar$, and $r$ from
\cref{eq:opt:phi-udiamond,eq:opt:obligatory-constant}.
...
For every finite $u\in[\varphi+2,\infty)$, the formally specified algorithm
...
\[
 \bar p_i=\min\{p_i,u-1\}.
\]
By \cref{eq:optional-effective-length,lem:offline}, the clairvoyant revealing
...
therefore uniform over all $u\ge \zstar$.  This includes the last transition
from $x=1$ to $x=0$.
...
At $u=\zstar$, one has $c=r$, $m=0$, and
$Z(r)=\zstar$.  The cap reserve is identically zero, while
\eqref{eq:opt:initial-calibration} remains valid.  If $u$ is increased
above $\zstar$, the only adverse fifth-endpoint bracket
...
only on $r$, for which every ordinary or capped transition with $u\ge \zstar$
satisfies
\[
...
\begin{equation}
 \ALG\le(1+r)\OPT+B n
 \qquad\text{for every finite }u\in[\zstar,\infty),
