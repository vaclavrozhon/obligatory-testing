...
\begin{proof}
Adjacent exchanges place as many
...
has competitive excess at least that of the original instance, and the
binary bound proves the claimed inequality.
\end{proof}
...
$p\in\{0,1\}$, and a suffix-deferred job to $p\in\{1^+,u\}$.  For the
suffix-deferred reduction, on $1<p<u-1$ the deferred--deferred term has coefficient
...
$x=a+d$ and write $F(x)=A-R O$ after optimizing the other variables.  Retain
the stationary point from the preceding claim,
\[
 \alpha=\frac{s^2+s+1}{s^2+s+1+sR}.
\]
The derivative of the gap in $t$ is $1-b-R(x+t)$.  Thus its maximizing
value is $(\chi-x)_+$, where the critical mass is
\[
 \chi=\frac{1-b}{R}
...
discussed; it is tangent to zero at $x=\alpha$.  For $x<\chi$, substitution
gives
\begin{equation}
 A-R O
...
outcome at the raw cap cannot be treated as an ordinary deferred outcome.
...
\medskip
\noindent\textbf{Claim 3 (The ordinary-endpoint bank).}
...
\medskip
\noindent\textbf{Claim 4 (The cap reserve).}
Define
\begin{equation}
