...
\label{thm:upper}
Let $z>1$ be the solution of $z-3=\ln z$, and set
\[
 R=1+\rho,
 \qquad
...
\label{eq:upper-alg-pair}
 ...
 \end{cases}
\end{equation}
The obligatory specialization \cref{eq:opt-pair} of the common offline pair
identity gives
\begin{equation}
...
\[
 f(0)=\frac12-\frac{\rho}{4}
       \ln\!\left(\frac{\rho+2}{\rho}\right).
\]
Thus $f(0)=\rho/2$ is equivalent to
\eqref{eq:upper-root-rho}, or, after setting $z=(\rho+2)/\rho$, to
$z-3=\ln z$.  Thus both the threshold and the constant $R$ are forced by
...
estimate to every input with nonnegative processing times, proving
\cref{thm:upper}.
