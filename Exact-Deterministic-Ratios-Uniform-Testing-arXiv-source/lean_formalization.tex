...
admissible \(n\)-job input \(I\), and a finite execution with no internal
administrative transition left---called \emph{settled} in the Lean
...
most \(2n+1\) internal action-selection steps (the ``analysis fuel'' in the
Lean definition), rather than merely assigning a cost to a partial
transcript.  Consequently,
...
machine-checked mixed-regime upper bound follows the reserve potential for
the endpoint $p=u$; the bubbling argument in
