...
\texttt{exactCurve u}
  & \(\RdetRO(u)\)
  & the six interval formulas in \cref{eq:opt:full-curve}, including all five
    verified joins \\
\texttt{RStar}
  & \(\RdetOT\)
...
all jobs are released at time zero: a strategy does not idle while a legal
operation is available.  Processing times and comparisons are
exact real numbers.

\subsection{Expansion of the lower and upper fields}

For either formal model parameter \texttt{Cap.finite u} or
\texttt{Cap.infinite}, and a candidate value \(R\),
\texttt{FixedSizeLowerBound} expands to the following statement.  For every
sequence of deterministic strategies \(\mathcal B_n\), every
\(\eps>0\), and every \(N\), there exist \(n\ge N\), a genuine fixed
admissible \(n\)-job input \(I\), and a finite execution in which all internal
bookkeeping transitions have finished (the Lean predicate
\texttt{settled}), such that
either the execution leaves a job incomplete or
\begin{equation}
  (R-\eps)\cdot\OPT(I)\le \ALG_{\mathcal B_n}(I).
...
\begin{equation}
 ...
 \ALG_{\mathcal A_n}(I)\le (R+\eps)\cdot\OPT(I).
 \label{eq:lean-upper-expansion}
\end{equation}
The accompanying completion field proves that every job is done after at
most \(2n+1\) choices of the next action.  This number is the \texttt{fuel}
bound in the Lean definition.  Thus the theorem analyzes a complete run,
not merely a partial transcript.  Consequently,
...
constructs a stronger \texttt{FixedUpperCertificate}: for the chosen formal
\texttt{Cap} parameter it
contains a constant \(C\ge0\) such that, for every \(n\) and every fixed
input,
\begin{equation}
  \ALG_{\mathcal A_n}(I)\le R\cdot\OPT(I)+Cn.
  \label{eq:lean-linear-certificate}
\end{equation}
Since every effective length is at least one in obligatory testing and at
least $\min\{u,1\}$ in the two optimization models with finite
raw-execution cap $u$,
\cref{eq:unified-opt-prefix} then implies
\eqref{eq:lean-upper-expansion}.  For \(u\ge\uDet{5}\), the separately verified
proof for the plateau uses the same strategy and the same \(C\), independent
...
only exposes one strategy after the formal \texttt{Cap} parameter has been
fixed.
...
\texttt{exactCurve} is literally the piecewise formula in
...
effective lengths and that the transcript cost is exactly the sum of each
operation length times the number of jobs unfinished during that operation.

\subsection{Proof boundary}

The root Lean module imports the complete lower- and upper-bound assemblies.
The public theorem does not assume any unproved property of the formal
execution interface, and the
...
noncomputable choices of exact real parameters.  Thus it proves mathematical
properties of strategies with exact real parameters; it does not extract a
floating-point implementation.

The theorem recorded here is specifically the size-asymptotic result.  It
does not assert the pointwise inequality \(\ALG\le R\cdot\OPT\) without a
remainder.  Its checked remainder is the linear term in
\eqref{eq:lean-linear-certificate}, and is therefore $o(n^2)$.  The
machine-checked upper bound on $[\uDet{4},\uDet{5}]$ uses the additional
potential for jobs with $p=u$; \cref{app:cap-bubbling} gives an independent
