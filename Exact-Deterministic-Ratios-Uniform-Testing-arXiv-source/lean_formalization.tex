\section{Machine-checked main theorem and its correspondence}
...
explains how its formal predicates correspond to the mathematical statements
in \cref{thm:main-intro,thm:optional-intro}.  The development uses the
...
\texttt{exactCurve u}
  & \(\V(u)\)
  & the six branches in \cref{eq:intro-optional-curve}, including all five
    verified joins \\
\texttt{RStar}
  & \(\Rstar\)
...
\subsection{Correspondence with the two main theorems}
...
\cref{thm:optional-intro}, while its upper field supplies a completing
strategy and the matching upper direction.  The definition of
\texttt{exactCurve} is literally the six-way case split in
\cref{eq:intro-optional-curve}; the Lean development also proves its joins,
continuity, plateau, and global maximum at \(\udiam\).

The second conjunct substitutes \(R=\texttt{RStar}=\Rstar\) in the
obligatory model.  Its lower field is
\cref{thm:main-intro}\textup{(i)}, and its completing upper strategy together
with \eqref{eq:lean-linear-certificate} is
\cref{thm:main-intro}\textup{(ii)}.  The common formal infrastructure also
