...
We now keep obligatory tests but change the adversary.  It fixes the labelled
vector $p\in[0,\infty)^n$ before the algorithm draws its random bits.  The
multiset is not announced and no upper bound on the processing times is
assumed.  The goal of this section is to prove \cref{thm:rand-obligatory},
...
We shall also use the following tail-safe estimate.  Direct expansion gives
\begin{equation}
 P-O=\frac{h+am-K_E}{2}.
...
the finite placement distribution shows that one fixed labelling attains at
least this average.  That labelling is chosen before the private seed and is
therefore an oblivious input.  Together with \cref{eq:rand-final-upper}, this
