\subsection{Harmonic core and matching lower bound}
\label{sec:lower}

This subsection proves the lower bound in one self-contained calculation.
Its notation is local to this subsection.  The construction
...
\begin{align}
 \frac{P_K}{\xi^2}&\longrightarrow I(\alpha)
   ...
   \nonumber\\
 &=\int_0^\alpha(\alpha-t)\bigl(1+\log(1+t)\bigr)\,dt
   \nonumber\\
 &=\frac12(1+\alpha)^2\log(1+\alpha)
   -\frac\alpha2-\frac{\alpha^2}{4}.
 \label{eq:lower-I}
\end{align}
The second integral form follows either by integration by parts or by summing
the limiting processing-time curve $1+\log(1+t)$ against the remaining
...
\[
  1+\zstar+\zstar\log\zstar=(\zstar-1)^2,
\]
and therefore
\[
  R(\zstar)=1+\frac{2}{\zstar-1}=\Rstar.
\]
...
\cref{eq:lower-harmonic}; then $Q_K/D_K>\Rstar-\eps/2$.
...
$\Rstar-\eps$ for all sufficiently large $H$.  Finally, the replay argument
...
every deterministic algorithm for the cap-free endpoint, there are scales
$M_\nu\to\infty$ and finite rational multilevel instances with
$sM_\nu+o(M_\nu)$ jobs such that
\begin{equation}
 \frac{\OPT}{M_\nu^2}\longrightarrow
 O_0=\frac{s^2}{4}(1+t^{-2}+2\log t),
 \qquad
...
\begin{equation}
 \frac1{M_\nu}\sum_{j:p_j>0}p_j\longrightarrow L=s\log t,
 \label{eq:opt:harmonic-processing-mass}
\end{equation}
and their largest processing time converges to $1+\log t<2$.
...
$1+\log(1+x)$ over the positive mass gives $L=s\log t$, and
\cref{eq:lower-harmonic} gives $p_0\to1+\log t$.  Rational approximation,
