...
pilot sample to choose the parameters of $\textsc{FluidSPT}$.
...
Thus a short type absent from the pilot still receives the correct action if
...
\begin{claim}[Learning from a pilot sample]
...
quota are fixed from the pilot before an unused hidden value is seen;
selector marks are drawn only after a test reveals their grid class.  Empty
pilot cells cause no undefined behavior because the canonical threshold
extension above defines their marks and queue positions on the full grid.
