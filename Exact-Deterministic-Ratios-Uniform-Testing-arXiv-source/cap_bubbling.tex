\section{Independent proof by moving capped jobs to a prefix}
...
argument is also a useful audit of the capped type in the five-endpoint
reduction \cref{eq:opt:five-endpoints}: it never
inserts $p=u$ into the formula for a deferred noncapped type before the caps have
actually been moved.

Recall the endpoint symbols from \cref{eq:opt:five-endpoints}: $Z$ is an
actual zero, $E$ a positive value tending to zero, $M$ an immediate
threshold value, $Q$ a deferred value just above the threshold, and $U$ a
capped value $u$.  A word in these symbols records their reveal order.
...
Work first in normalized fluid mass.  Immediately before neighboring symbols
...
caps, gives the two nontrivial swap differences
\begin{align}
 UM-MU
...
neighboring swap valid even while other caps remain to the right.  Repeatedly
moving a cap left in this way---the ``bubbling'' in the section title---puts
all caps into one prefix.

After that prefix there is no future cap.  Let $e$ be the preceding $E$-mass,
...
\[
 ...
 \quad k=x+d-Rb.
\]
The four swap differences involving only noncapped types are
\begin{align}
...
\end{equation}
Here $U^\mu$ denotes a block of capped mass $\mu$, and the star denotes an
...
entire continuation containing only the noncapped types.  If the cap prefix
has mass $\mu$, then
after it $x=1-\mu$, $N=N_{\mathrm{def}}=\mu$, and
$y=-c\mu/(1-\mu)$.  Its contribution from pairs of capped jobs plus its worst
...
unit interval.  At $c=r,m=0,u=\uDet{5}$, the same calculation is maximized at
...
affected.  All capped and noncapped swaps among the retained jobs then
...
last block, so no cap remaining later in the word invalidates
...
\]
which independently proves the required size-asymptotic inequality for the
two upper bounds above.
