...
\subsection{Fluid schedules: from test modules to completion curves}
\label{sec:common-fluid-envelopes}

The instance-optimality proofs later in the paper repeatedly solve the same
fluid scheduling problem.  We decide that a fraction $q$ of the jobs will be
tested.  Some short outcomes are processed together with their tests, the
remaining tested jobs are deferred, and the untested jobs are handled by the
other mode available in the model.  The question is how these actions should
be ordered.

At the fluid scale, the $n$ jobs are replaced by one unit of divisible job
mass: a class containing $30\%$ of the jobs has mass $0.3$.  Work is divided
by $n$ as well, so processing that class for time $p$ uses $0.3p$ units of
work.  The leading $n^2$ term of total completion time becomes the area under
the fraction of unfinished jobs.

The solution has two steps.  First, we combine tests with precisely those
short outcomes whose immediate processing improves the number of completions
per unit of work.  This gives one optimal \emph{test module}.  Second, we view
this module, the deferred outcomes, and the other completion mode as
divisible blocks and order them shortest first.  The purpose of this
subsection is to justify these two steps and to record the resulting
completion curve.

We begin with the first step.  Let $D$ be a distribution of job lengths with
finite mean.  A test module tests one unit of job mass and immediately
processes a chosen fraction of every positive length class.  If
$g(p)\in[0,1]$ is the chosen fraction of class $p$, the module completes mass
$a_D(g)$ using work $w_D(g)$, where
\begin{equation}
...
number of completions per unit of work.
...
\paragraph{From one module to a full schedule.}
Choosing the best test module does not yet specify a schedule: we still have
to place it relative to the jobs deferred after testing and to jobs completed
by another mode.  At the fluid scale, each kind of work is described by its
completion capacity $c_k$ and the work $v_k$ needed per completion.  For
example, a block $c_k\delta_{v_k}$ can complete mass $c_k$, with each unit of
that mass costing $v_k$ work.
...
\paragraph{The completion curve for a fixed tested fraction.}
We can now return to the scheduling problem from the beginning of the
subsection.  Fix the final tested fraction $q$.  The remaining fraction
$1-q$ is completed by a mode that costs $v$ work per job.  In revealing
optimization this is raw execution and $v=u$; in blind execution the fluid
cost of a blindly executed job is the mean processing time.

Assume for the moment that $D$ has finite support and put
$d_p=D(\{p\})$.  After some initial part of a schedule, let
$t$ be the mass already tested, $b$ the mass completed by the other mode,
and $c_p$ the tested mass of positive class $p$ that has also been processed.
Thus the completed mass is
\[
 d_0t+b+\sum_{p>0}c_p,
\]
...
$d_pt$ mass of any revealed class.  Consequently, the largest mass that can
be completed after work $x$ is
\begin{equation}
...
\end{equation}
At first sight, this linear program allows many interleavings of tests and
processing.  The maximum-density lemma reduces all of them to three familiar
blocks:
\[
...
has the capacity and cost of the other mode.  The next lemma says that their
shortest-first completion curve is exactly the envelope above, not merely a
bound on it.
...
\begin{proof}
We prove the two inequalities in the curve identity separately.
...
Conversely, order the three kinds of fractional items by work per completion,
placing the test module before a residual item in case of a tie.  The
independent item can be run whenever this order requires it.  A fraction of
the test-module block is realized by the corresponding fraction of tests and
selected outcomes.  Every residual length is at least $\tau_D$, so by the
time a residual item is reached, all tests needed to expose it have already
run.  This realizes the shortest-first curve at every work budget and proves
the curve identity.  Its area is given by
