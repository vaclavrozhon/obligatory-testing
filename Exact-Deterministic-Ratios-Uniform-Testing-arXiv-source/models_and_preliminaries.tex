...
\begin{enumerate}[label=\textnormal{(\roman*)},leftmargin=*]
 \item Reserve a $q$-fraction of every processing-time group for testing and
       assign the remaining $1-q$ fraction to the other completion mode.
...
During an infinitesimal amount $dx$ of work, every job in the unfinished
mass $1-C(x)$ waits for time $dx$.  The total fluid completion cost is
therefore the area under the unfinished fraction.  For any completion curve
$C$, write
\begin{equation}
