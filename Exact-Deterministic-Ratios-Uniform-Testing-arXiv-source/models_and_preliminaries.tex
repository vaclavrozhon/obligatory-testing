...
the oblivious adversary fixes the labeled vector first, and the algorithm
...
Mathematically we allow arbitrary private probability spaces.  The Lean
exports in \cref{sec:formalization} quantify over finite uniform seed types.
This does not change the paper ratios: on each finite input family used in a
bound, only finitely many transcript decisions and their costs are relevant,
and conditioning followed by finite-support approximation changes every
expected cost by an arbitrarily small amount.  The appendix records this
seed convention explicitly and marks the two symmetrization corollaries that
are not themselves exposed as Lean theorems.
...
\subsection{Predictable sampling}
\label{sec:probabilistic-tools}

The standard concentration estimates used below are collected in
\cref{sec:standard-concentration}.  We retain here the specialized estimate
that permits data-dependent weights while sampling a uniformly permuted
finite population.
