...
\end{equation}
We call the algorithm below \emph{stationary} because the set $E$ and the
immediate/deferred rule are fixed before the random test order is exposed;
the rule does not change with the observed history.
...
$W_E=e+\sum_{i\in E}p_i$.  The initial random test phase has one block per job: an
...
\end{equation}
Adding the two displays and collecting the one-job contributions gives
...
transfer its chosen algorithmic parameters back to the true distribution.
Here and below, a \emph{template} means a fixed choice of the few fluid
parameters used by an algorithm, such as a threshold or tested fraction.
The next deterministic lemma isolates this elementary step.
...
Fix values $p_1,\ldots,p_n\in[0,L]$, partition $[0,L]$ into cells
$C_0,\ldots,C_s$, and let
$d_j=n^{-1}|\{i:p_i\in C_j\}|$ be the empirical mass of cell $C_j$.  Put
$\mu=n^{-1}\sum_i p_i$.  Place these $n$ entries uniformly on the public labels,
fix a deterministic algorithm, and let $X_k$ be the value encountered at first
touch $k$.  If $A_k$ is the predictable indicator that this touch tests the
job and reveals its value before processing, then for every
$\delta,\eta\in(0,1)$, with probability at
...
using work $x$.  Thus, for example, $\pi$ may record the tested fraction
$q$.  Define
\begin{equation}
...
Consider two schedules on the same $n$ jobs that finish every job and never
idle while an operation is available.  Suppose each has total work at most
$Wn$ and one is obtained from the other by changing
