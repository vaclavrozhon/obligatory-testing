\section{Randomized common-upper testing: the exact four-piece curve}
\label{sec:random-common-upper}

The previous section identified the optimal leading online cost for every
fixed bag in the common-upper model.  We now compare that cost with the
clairvoyant optimum and maximize over all bags.  The cap $u$ is fixed, but
...
curve theorem \cref{thm:intro-common-upper-curves}.  Let
...
advance knowledge of the bag, and binary random labelings give matching Yao
...
$\mathcal A_{n,u}$ such that every fixed labeled input with empirical
...
makes the law identical for every fixed public labeling.
\end{proof}
...
algorithm, even one told $M_n$, some fixed labeling satisfies
\begin{equation}
 \frac{\EE\ALG}{n^2}\ge
 \begin{cases}
