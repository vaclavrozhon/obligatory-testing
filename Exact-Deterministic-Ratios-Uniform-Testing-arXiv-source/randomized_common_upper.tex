...
\begin{equation}
 ...
 \le \RrandRO(u)\cost(\OPT_u,p)+\eps_u(n)n^2.
...
\begin{equation}
 \EE\cost(\mathcal A,p)
 \ge\bigl(\RrandRO(u)-\eps\bigr)\cost(\OPT_u,p).
...
\begin{equation}
 \mathcal T_u(D)=
 \begin{cases}
  \displaystyle \tau+\int_\tau^u S_D(t)^2\,dt,&\tau\le u,\\[6pt]
  \tau,&\tau>u.
 \end{cases}
...
\[
 K_L=\tau h^2+\int_\tau^uS_D(t)^2\,dt,
 \qquad
...
instance-specific ratio $\PhiRO(D)/\Omega_u(D)$.  Interior values of $q$ can
...
$[0,\tau]$.  Since $S_D$ is nonincreasing, its average on every initial
...
\end{equation}
and $S_D(t)\le y$ almost everywhere for $t>\tau$.

Replace $S_D$ on $(0,\tau)$ by the constant $y$ and leave its tail unchanged.
