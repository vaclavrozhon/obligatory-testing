...
and \cref{cor:empirical-offline} give
\begin{align}
 \Omega_u(D)
...
\subsection{Removing the announcement by sampling}
...
prefixes.  The empirical-minimization lemma
\cref{lem:empirical-minimization} therefore loses at most twice the
right-hand side of \eqref{eq:rcu-histogram-error}.  Zero-preserving upward
rounding changes the fluid benchmark by $O_u(1/d)$ by
\cref{lem:zero-preserving-transport}.
...
\cref{lem:adaptive-first-touch}, the analytically exposed occurrence word is
a uniform permutation, and the test/raw choice before its next entry is
predictable.
