\section{Randomized common upper bounds: the exact four-piece curve}
\label{sec:random-common-upper}

We return to the finite common-upper model and allow private randomization
against an oblivious adversary.  The cap $u$ is fixed throughout this
section, but the algorithm is not told the processing-time multiset.  This
section proves \cref{thm:random-common-upper}, the rigorous randomized half
of the informal curve theorem \cref{thm:intro-common-upper-curves}.  Let
$\urand>1$ be the unique root of
\begin{equation}
...
\begin{theorem}[Randomized common-upper curve]
...
bag.  Here we compare that value with the clairvoyant minimum kernel and
...
scalar maximizations give the four branches above.  A sublinear pilot removes
advance knowledge of the bag, and binary random labellings give matching Yao
lower bounds.  The next subsection starts the formal calculation.
...
and \cref{lem:offline} give the leading offline coefficient
\begin{align}
 \Omega_u(D)
 &\defeq \frac12\EE\min\{q_u(P),q_u(P')\}
  =\frac12\left(1+\int_0^s H(t)^2\,dt\right),
 \label{eq:rcu-offline-fluid}\\
 \OPT_u(M)
 &=n^2\Omega_u(D_M)+O_u(n)
 \label{eq:rcu-offline-finite}
\end{align}
for every empirical distribution $D_M$.

We next recall the maximum-density stationary policy in a form suited to the
cap.  For a processing-time prefix of mass $a$ and first moment $m$, its
completion density is $a/(1+m)$.  Choose a maximum-density prefix, include
all zeros, and put
\begin{equation}
 \tau=\frac{1+m}{a}.
 \label{eq:rcu-tau-density}
\end{equation}
The one-point addition/removal test puts every value below $\tau$ in the
prefix and every value above $\tau$ outside it; an atom at equality may be
split.  Equivalently,
\begin{equation}
 \EE(\tau-P)^+=1,
 \qquad
 \int_0^\tau H(t)\,dt=\tau-1.
 \label{eq:rcu-threshold-equation}
\end{equation}
The equation has a unique solution $\tau\ge1$.
...
finite-cap analogue of \cref{lem:rand-stationary-cost}.
...
\begin{equation}
 \mathcal T_u(D)=
 \begin{cases}
  \displaystyle \tau+\int_\tau^u H(t)^2\,dt,&\tau\le u,\\[6pt]
  \tau,&\tau>u.
 \end{cases}
...
\begin{proof}
Let $h=1-a$ be the residual mass and let $K_L$ be the minimum kernel of two
residual draws, with their subprobability masses left unnormalized.  The
random-order pair calculation gives the stationary coefficient
\begin{equation}
 (1+m)\left(1-\frac a2\right)+\frac{K_L}{2}.
 \label{eq:rcu-stationary-pair}
\end{equation}
If $\tau\le u$, the threshold property gives
\[
 K_L=\tau h^2+\int_\tau^uH(t)^2\,dt,
 \qquad
 (1+m)\left(1-\frac a2\right)
 =\frac\tau2(1-h^2).
\]
Their sum is one half of the first line of
\eqref{eq:rcu-stationary-survival}.  If $\tau>u$, the whole distribution is
in the prefix and \eqref{eq:rcu-stationary-pair} equals $\tau/2$.  Raw
...
\begin{equation}
 2\Phi_u(D)\le\min\{u,\mathcal T_u(D)\},
 \label{eq:rcu-Phi-endpoint-upper}
\end{equation}
and \cref{eq:rcu-announced-ratio} is an upper bound on the
instance-specific ratio $\Phi_u(D)/\Omega_u(D)$.  Interior values of $q$ can
strictly improve individual bags; the point of the next two subsections is
...
first that $1\le\tau\le u$ and put
\begin{equation}
 y=\frac{\tau-1}{\tau}.
 \label{eq:rcu-y}
\end{equation}
By \eqref{eq:rcu-threshold-equation}, $y$ is the average of $H$ on
$[0,\tau]$.  Since $H$ is nonincreasing, its average on every initial
subinterval is at least $y$.  Thus Cauchy--Schwarz gives
\begin{equation}
 \int_0^aH(t)^2\,dt\ge ay^2
 \qquad(0\le a\le\tau),
 \label{eq:rcu-initial-cs}
\end{equation}
and $H(t)\le y$ almost everywhere for $t>\tau$.

Replace $H$ on $(0,\tau)$ by the constant $y$ and leave its tail unchanged.
...
If $u-1\le\tau\le u$, the offline integral lies entirely in the flat part,
and $H(t)^2\le y^2$ on the remaining tail.  It follows that
\begin{equation}
 \frac{\min\{u,\mathcal T_u(D)\}}
      {1+\int_0^sH^2}
 \le
 B_u(\tau)
 \defeq
 \frac{\tau+(u-\tau)y^2}{1+(u-1)y^2}.
 \label{eq:rcu-B-first}
\end{equation}
The numerator displayed on the right is at most $u$, so the raw minimum is
inactive.

Now suppose $\tau<u-1$.  Put
\begin{equation}
 L=u-1-\tau,
 \quad E=\int_\tau^{u-1}H(t)^2\,dt,
 \quad G=\int_{u-1}^uH(t)^2\,dt.
 \label{eq:rcu-EG}
\end{equation}
Monotonicity gives
\begin{equation}
...
\begin{equation}
 \frac{\min\{u,\mathcal T_u(D)\}}
      {1+\int_0^sH^2}
 \le
 \frac{\tau+\dfrac{u-\tau}{u-1-\tau}E}
      {1+\tau y^2+E}.
 \label{eq:rcu-linear-fractional-E}
\end{equation}
For fixed $\tau$ this is linear-fractional in $E$, so its derivative has a
constant sign.  Its maximum on $[0,Ly^2]$ occurs at an endpoint.  The two
endpoint values are
\begin{align}
 A(\tau)&\defeq\frac{\tau}{1+\tau y^2}
              =\frac{\tau^2}{\tau^2-\tau+1},
 \label{eq:rcu-A}\\
 B_u(\tau)&=\frac{\tau+(u-\tau)y^2}{1+(u-1)y^2}.
 \label{eq:rcu-B}
\end{align}
They are attained by rectangular survival functions: respectively a binary
distribution on $\{0,\tau\}$ and one on $\{0,u\}$.

It remains to cover $\tau>u$.  Equation
\eqref{eq:rcu-threshold-equation} and the zero extension of $H$ give
$\int_0^uH=\tau-1\ge u-1$.  The average of a nonincreasing function on
$[0,u-1]$ is no smaller than its average on $[0,u]$, and hence
\begin{equation}
 \int_0^{u-1}H(t)^2\,dt
 \ge (u-1)\left(\frac{\tau-1}{u}\right)^2
 \ge\frac{(u-1)^3}{u^2}.
...
sign there exactly once; its unique root above one is $\urand$ from
\eqref{eq:intro-random-transition}.  Hence
\begin{align}
 \max_{1\le\tau\le u}B_u(\tau)
 &=B_u(u)=\frac{u^3}{u^3-2u^2+3u-1}
       =\frac{u^3}{u^2+(u-1)^3},&&u\le\urand,
 \label{eq:rcu-B-at-u}\\
 &=B_u(\tau_*(u))
       =1+\frac1{2(\sqrt u-1)},&&u\ge\urand.
...
\]
On $3\le u\le\urand$, one has $B_u(u)>4/3$.  On
$\urand\le u\le25/4$, \eqref{eq:rcu-B-at-star} is at least $4/3$, with
equality exactly at $u=25/4$; afterwards $A(2)=4/3$ is larger.  Combining
...
continuity at $\urand$, and the square-root branch equals $4/3$ at
$u=25/4$.  Differentiating \eqref{eq:rcu-B-at-u} shows that its only
interior maximum above one occurs at $u=(3+\sqrt3)/2$; substitution gives
\eqref{eq:intro-random-common-maximum}.

\subsection{Learning the announced policy}

The policy used for the upper bound must not know $D$.  The next lemma
removes this announcement at lower-order cost.

\begin{lemma}[Sublinear-pilot transfer]
\label{lem:rcu-pilot-transfer}
For every fixed $u>1$, there are randomized nonanticipating policies
$\mathcal A_{n,u}$ such that every fixed labelled input with empirical
distribution $D$ satisfies
\begin{equation}
 \EE\ALG_{\mathcal A_{n,u}}
 \le \frac{n^2}{2}\min\{u,\mathcal T_u(D)\}
      +O_u(n^{11/6}).
 \label{eq:rcu-pilot-bound}
\end{equation}
\end{lemma}

\begin{proof}
Let $k=\lfloor n^{2/3}\rfloor$, $d=\lfloor n^{1/3}\rfloor$, and
$\eta=u/d$.  Keep zero as a separate category and round each positive value
upward to one of the $d$ grid endpoints.  The algorithm privately permutes
the labels, tests the first $k$, and forms the sample histogram.  On that
histogram it compares the raw template with every stationary grid-prefix
template.  It applies a minimizing template to the remaining labels; in the
stationary case, prefix outcomes are processed immediately and the residual
tail in SPT order.  Sampled positive jobs are inserted in the corresponding
known part of the schedule.

Let $\widehat D$ and $D^{\rm g}$ be the sample and population grid
histograms.  Hypergeometric variance and Cauchy--Schwarz over the $d+1$
categories give
\begin{equation}
 \EE\|\widehat D-D^{\rm g}\|_1
 \le\sqrt{\frac{d+1}{k}}=O(n^{-1/6}).
 \label{eq:rcu-histogram-error}
\end{equation}
For a fixed prefix, the stationary coefficient is a quadratic polynomial in
the category masses, with coefficients bounded in terms of $u$.  It is
therefore $O_u(1)$-Lipschitz in total variation, uniformly over all grid
prefixes.  The empirical-minimization sandwich loses at most twice the
right-hand side of \eqref{eq:rcu-histogram-error}.  Zero-preserving upward
rounding changes every normalized work and pair kernel by $O_u(1/d)$.

Finally, placing the sample before the main random-order template affects at
most $O(nk+k^2)$ bounded operation--job pairs.  The already tested sample
has the same bound when raw is selected.  After normalization by $n^2$, the
total error is
\[
 O_u\left(\sqrt{d/k}+1/d+k/n\right)=O_u(n^{-1/6}).
\]
This proves \eqref{eq:rcu-pilot-bound}.  The initial private permutation
makes the law identical for every fixed public labelling.
\end{proof}

Together with \eqref{eq:rcu-offline-finite} and
\eqref{eq:rcu-announced-upper}, \cref{lem:rcu-pilot-transfer} yields
\begin{equation}
 \EE\ALG_{\mathcal A_{n,u}}
 \le\Rrand(u)\OPT_u+O_u(n^{11/6}),
 \label{eq:rcu-final-upper}
\end{equation}
which is \eqref{eq:intro-random-common-upper} with
$\eps_u(n)=O_u(n^{-1/6})$.  For $0<u\le1$, raw execution is exactly
clairvoyantly optimal, so no learning is required.
...
binary random-labelling envelope.
...
algorithm, even one told $M_n$, some fixed labelling satisfies
\begin{equation}
 \frac{\EE\ALG}{n^2}\ge
 \begin{cases}
...
\begin{proof}
First fix a deterministic policy and place the occurrences uniformly on the
public labels.  List labels in the order in which the policy first touches
them, by either a test or a raw execution, and expose analytically also the
hidden value of a raw-touched occurrence.  The map from placements to this
occurrence word is a bijection: equal word prefixes give equal observable
histories, hence the same next label and touch kind, and induction recovers
the whole placement.  The word is therefore a uniform permutation, and the
test/raw choice before its next entry is predictable from the preceding
word.

Take $\delta_n=n^{-1/4}$.  Uniform-prefix hypergeometric concentration,
followed by maximal Azuma--Hoeffding for the predictably selected tests,
gives one event of probability $1-o(1)$ on which every first-touch prefix
...
\begin{equation}
 ...
 \Delta_n,
 \qquad
 \Delta_n=O(\delta_n^{-1}\sqrt{n\log n})=o(n).
...
We now select the binary masses.  If $1<u\le\urand$, take
\begin{equation}
 x=\frac{u-1}{u}.
...
\end{equation}
If $\urand\le u\le25/4$, take
\begin{equation}
 x=\frac1{\sqrt u-1}.
 \label{eq:rcu-lower-x-stationary}
\end{equation}
The definition of $\urand$ is precisely the point from which
...
For every $u\ge25/4$ and every randomized algorithm, arbitrarily large even
$n$ admit a fixed labelling with $n/2$ jobs of value zero and $n/2$ jobs of
value two such that
\begin{equation}
...
one.  This proves the online estimate after the $o_u(n^2)$ repairs and the
finite Yao selection.  Since $u>3$, the two effective lengths are one and
three; their equally weighted minimum kernel gives offline coefficient
$3/4$.
\end{proof}
