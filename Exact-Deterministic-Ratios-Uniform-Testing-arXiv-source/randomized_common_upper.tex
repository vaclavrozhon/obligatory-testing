...
advance knowledge of the input, and shuffled binary vectors give matching
lower bounds.
...
clairvoyant job of length $p$ has effective length $1+\min\{s,p\}$, layer cake and the
symmetric pair identity \cref{eq:unified-opt-symmetric} give
\begin{align}
...
that the two endpoint algorithms already give the sharp value after maximizing
...
It remains to show that the two endpoint algorithms used above cannot be
