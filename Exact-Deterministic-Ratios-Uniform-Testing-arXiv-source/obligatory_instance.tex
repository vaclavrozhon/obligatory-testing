...
may implement the selector with whole job occurrences.  Sort the jobs by
processing time and, among prefixes $E$ containing all zero jobs, maximize
\[
 \frac{|E|/n}{1+n^{-1}\sum_{i\in E}p_i}.
\]
The addition/removal test for this ratio includes precisely the occurrences
below $\tau_D$ and excludes those above it.  Adding or removing an occurrence
with $p_i=\tau_D$ preserves the ratio, so ties may be placed on either side.
...
$0\le\PhiOT(D^+)\le(L+1)/2$.

We also need to compare the rounded and original fluid values.  Couple every
processing time with its rounded copy and use the same fluid decisions for
both.  Zeros are unchanged and therefore complete at the same tests.  At
every prefix, at most one unit of job mass has received processing, so the
accumulated work changes by at most the rounding mesh.  The two
unfinished-mass curves are consequently contained in horizontal shifts of
one another.  Integrating and exchanging the two inputs proves that any
zero-preserving coupling with displacement at most $h$ satisfies
\begin{equation}
 |\PhiOT(D)-\PhiOT(D')|\le h.
 \label{eq:ot-rounding-stability}
\end{equation}
Applying this with $D[p]$ and $D^+$ changes the fluid value by at most $h_n$.
Because
...
\cref{eq:ot-rounding-stability} gives
\[
...
\end{equation}
Put $L_0=3(L+2)$.  Applying \cref{eq:ot-template-lipschitz} first to
$\widehat J$ and then to an optimal prefix for $D^{\rm rem}$ gives
\[
 F_{\mathsf{OT},D^{\rm rem}}(\widehat J)
 \le \PhiOT(D^{\rm rem})
      +2L_0\|\widehat D-D^{\rm rem}\|_1.
\]
Applying the same bound to an optimal prefix for $D^+$ gives
$\PhiOT(D^{\rm rem})\le
\PhiOT(D^+)+L_0\|D^{\rm rem}-D^+\|_1$.
Moreover,
$\|\widehat D-D^{\rm rem}\|_1\le
\Delta+\|D^{\rm rem}-D^+\|_1$.
Average \cref{eq:ot-sample-first-instance}, use
\cref{eq:ot-learning-histogram}, and then use the rounding bound
