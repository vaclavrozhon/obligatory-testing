...
\end{equation}
Conversely, every $p\in[0,L]^n$ and every randomized policy $\mathcal A$,
even an announced policy, satisfy
\begin{equation}
 \EE\ALG_{\mathcal A}(p)
...
$[0,n^2]$ and an announced randomized policy $\mathcal A'$
such that
\begin{equation}
...
\begin{equation}
 ...
 \label{eq:ot-Phi}
\end{equation}
For $D=D[p]$ and a whole-occurrence maximizing prefix $E$, this is precisely
$F_{\mathsf{OT},D[p]}(E)$ from \cref{eq:stationary-leading-coefficient}.  The next
...
permutation $P_1,\ldots,P_n$ of the coordinates of $p$ by deferred decisions.  Equal-valued
...
policy has private-shuffle expected cost at least
\begin{equation}
...
average over the seed.  This is precisely the randomized cost under the
input convention.
