...
We next specialize the common transfer to blind execution.  A direct first
touch now runs the job for its actual processing time, so $r(p)=p$ and the
direct work per unit mass is the mean processing time.  The fluid optimum has
four blocks: a maximum-density testing block, tested medium jobs, jobs run
blindly, and tested long jobs.  We first state instance optimality and then
expand this four-block schedule into a one-variable quadratic.
...
\subsubsection{The explicit four-block benchmark}
...
$v_r(D)=\mu$, and \cref{eq:blind-fluid-blocks,eq:blind-Phi} show that the
common value $\Phi_{r,Q}(D)$ equals $\PhiBE(D)$.  The two conclusions and
the stated rate now follow from
\cref{eq:common-transfer-upper,eq:common-transfer-lower,eq:common-transfer-rate}.
\end{proof}

\subsubsection{Why all four blocks can be necessary}
...
\subsubsection{Why bounded processing times are necessary}

The common transfer assumes a fixed upper bound $L$.  For blind execution
this is essential even for obtaining any finite worst-case ratio.
